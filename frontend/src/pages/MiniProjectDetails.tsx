...
const API_URL =
  import.meta.env.VITE_API_URL || "https://skillverse-ogv1.onrender.com/api";
...
const MiniProjectDetails = forwardRef<any, MiniProjectDetailsProps>(
  ({ onHasUnsavedChanges, projectTitle, onBack }, ref) => {
    ...
    const handleRegrade = async () => {
      ...
      try {
        ...
        const response = await axios.post(
          `${API_URL}/mini-projects/regrade-project`,
          { projectTitle },
          { headers: { Authorization: `Bearer ${token}` } },
        );
        ...
        if (response.data) {
          ...
          alert(`Re-grading complete! Score: ${response.data.score}/100`);
        }
      } catch (error: any) {
        console.error("Regrade error:", error);
        alert(error.response?.data?.message || "Failed to re-grade project");
      } finally {
      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              score !== null && (
                <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span
                      className={`text-2xl font-bold ${score >= 70 ? "text-green-600" : "text-red-600"}`}
                    >
                      {score}/100
                    </span>
                  </div>
            ...
            {/* Re-grade button for submitted projects without score */}
            {status === "submitted" && (score === null || score === 0) && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-6 shadow-sm">
                <h2 className="text-lg font-semibold mb-3 text-yellow-900">
                  Grading Pending
                </h2>
                <p className="text-sm text-yellow-800 mb-4">
                  Your project was submitted but AI grading failed or is
                  incomplete. Click below to re-grade your submission.
                </p>
                <button
                  onClick={handleRegrade}
                  disabled={isRegrading}
                  className="w-full px-4 py-2 bg-yellow-600 text-white rounded-lg text-sm font-medium transition-all hover:bg-yellow-700 hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isRegrading ? "Re-grading..." : "Re-grade Project"}
                </button>
              </div>
            )}
          </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              ...
              <div className="flex items-center gap-2">
                <Code
                  ...
                  style={{
                    color:
                      project.language?.toLowerCase() === "python"
                        ? "#F59E0B"
                        : "#3B82F6",
                  }}
                  strokeWidth={1.5}
            ...
            <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-6 space-y-4">
                <div className="flex items-center justify-between pb-4 border-b border-gray-200">
                  ...
                  <div
                    className={`text-3xl font-bold ${score! >= 70 ? "text-green-600" : "text-red-600"}`}
                  >
                    {score}/100
                  </div>
                </div>
