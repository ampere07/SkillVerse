...
const API_URL =
  ...
  "https://skillverse-ogv1.onrender.com.com/api";
...
const MiniProjects = forwardRef<any, MiniProjectsProps>(
  ({ onHasUnsavedChanges, onNavigateToDetails }, ref) => {
    ...
    const [projectStatuses, setProjectStatuses] = useState<Map<string, string>>(
      new Map(),
    );
    const [projectScores, setProjectScores] = useState<Map<string, number>>(
      new Map(),
    );
    const [projectFeedback, setProjectFeedback] = useState<Map<string, string>>(
      new Map(),
    );
    const [showSurvey, setShowSurvey] = useState(false);
    const [surveyLanguage, setSurveyLanguage] = useState<string | undefined>(
      undefined,
    );
    const [showLanguageMenu, setShowLanguageMenu] = useState(false);
    const [currentLanguage, setCurrentLanguage] = useState<string>("java");
    const [showConfirmationModal, setShowConfirmationModal] = useState(false);
    const [pendingLanguage, setPendingLanguage] = useState<string | undefined>(
      undefined,
    );
    const [surveyCompletedLanguages, setSurveyCompletedLanguages] = useState<
      string[]
    >([]);
      ...
      const checkIfTrulyNew = async () => {
        ...
        if (isNewStudent) {
          try {
            ...
            const surveyCompletedLanguages =
              response.data.user.surveyCompletedLanguages || [];
            const userPrimaryLanguage =
              response.data.user.primaryLanguage || "java";
            ...
            // Show survey if current language hasn't been assessed yet
            if (!surveyCompletedLanguages.includes(userPrimaryLanguage)) {
    ...
    const fetchProjects = async (fromSurvey = false) => {
      try {
        ...
        const response = await axios.get(
          `${API_URL}/mini-projects/available-projects`,
          {
    ...
    const fetchCompletedTasks = async () => {
      try {
        const token = localStorage.getItem("token");
        const response = await axios.get(
          `${API_URL}/mini-projects/completed-tasks`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
        ...
        const completedThisWeekTitles = new Set(
          (response.data.completedThisWeek || []).map((task: any) =>
            task.projectTitle.toLowerCase(),
          ),
        );
        ...
        setConsecutiveDays(
          calculateConsecutiveDays(response.data.completedTasks),
        );
      } catch (error) {
    ...
    const getProjectStatus = (projectTitle: string) => {
      ...
      if (status === "completed" || completedTitles.has(titleLower))
        return "completed";
      return "not-started";
    };
        ...
        >
          <div className="flex flex-col items-center justify-center h-64 space-y-3">
            ...
            <p className="text-sm" style={{ color: "#757575" }}>
              This may take a moment if generating new projects
            </p>
          </div>
        </div>
      );
                  ...
                  <div className="absolute right-0 mt-2 w-56 bg-white border border-gray-200 rounded-lg shadow-lg z-50">
                    <button
                      onClick={async () => {
                        setShowLanguageMenu(false);
                        const otherLanguage =
                          currentLanguage === "java" ? "python" : "java";
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-xs" style={{ color: "#757575" }}>
                          Change to{" "}
                          {currentLanguage === "python" ? "Java" : "Python"}
                        </p>
                      </div>
                    </button>
                  </div>
                ...
                >
                  ...
                  {!isEarned && progress > 0 && (
                    <div className="mt-2">
                      ...
                      <p className="text-[9px] text-center mt-1 text-gray-500">
                        {Math.round(progress)}%
                      </p>
                    </div>
                  )}
          ...
          {(() => {
            ...
            return earnedCount === 0 ? (
              <div className="text-center py-4 mt-2">
                <p className="text-sm" style={{ color: "#757575" }}>
                  Complete projects to earn your first badge!
                </p>
              </div>
            ) : null;
          })()}
        ...
        {projects.length === 0 ? (
          <div className="bg-white border border-gray-200 rounded-xl p-12 text-center shadow-sm">
            ...
            <p className="text-sm mb-6" style={{ color: "#757575" }}>
              Complete your onboarding survey to get personalized AI-generated
              projects
            </p>
            <button
              onClick={() => fetchProjects()}
                ...
                >
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                    ...
                    {(status === "submitted" || status === "completed") &&
                      projectScores.get(project.title.toLowerCase()) !==
                        undefined && (
              ...
              <div className="px-6 py-6 space-y-4">
                <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
                  <div
                    ...
                    style={{
                      backgroundColor:
                        currentLanguage === "java" ? "#DBEAFE" : "#FEF3C7",
                    }}
                  >
                    <span
                      ...
                      style={{
                        color:
                          currentLanguage === "java" ? "#3B82F6" : "#F59E0B",
                      }}
                    >
                  ...
                  <div className="flex-1">
                    ...
                    <p
                      ...
                      style={{ color: "#212121" }}
                    >
                      {currentLanguage === "java" ? "Java" : "Python"}
                    </p>
                  </div>
                </div>
