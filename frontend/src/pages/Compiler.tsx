...
const JAVA_TYPES = ['String', 'Integer', 'Double', 'Float', 'Long', 'Boolean', 'Character', 'Byte', 'Short'];

const JAVA_CONSTANTS = ['true', 'false', 'null'];
...
const highlightJavaCode = (code: string, errors: CompilationError[] = [], warningLines: Set<number> = new Set()) => {
...
const highlightLineSegment = (text: string, lineIndex: number, startIndex: number) => {
  ...
  while (remaining) {
    ...
    // Match words (keywords, types, identifiers)
    const wordMatch = remaining.match(/^[a-zA-Z_]\w*/);
    if (wordMatch) {
      const word = wordMatch[0];
      let color = '#000000'; // Default black text color
      ...
      if (JAVA_KEYWORDS.includes(word)) {
        ...
        color = '#0000FF'; // Blue for constants
      } else if (word.match(/^[A-Z]/)) {
        color = '#267F99'; // Teal for class names (capitalized)
      }
...
const findMissingImports = (code: string): MissingImport[] => {
  ...
  // Find all class names used in the code
  const classMatches = code.match(/\b[A-Z]\w+\b/g) || [];
  
  for (const className of classMatches) {
    // Skip if already found or if it's the main class
...
const findLinesWithMissingImports = (code: string, missingImports: MissingImport[]): Set<number> => {
  const lines = code.split('\n');
  const linesWithWarnings = new Set<number>();
  
  missingImports.forEach(({ className }) => {
    lines.forEach((line, index) => {
      // Check if this line uses the class that needs an import
      const regex = new RegExp(`\\b${className}\\b`);
      if (regex.test(line)) {
        linesWithWarnings.add(index + 1); // Line numbers are 1-indexed
      }
    });
  });
...
const findUnusedImports = (code: string): Set<number> => {
  const lines = code.split('\n');
  ...
  lines.forEach((line, index) => {
    ...
    if (importMatch) {
      ...
      // Get the class name (last part after the last dot)
      const className = fullImport.split('.').pop();
      
      if (className && className !== '*') {
        // Check if this class is used anywhere in the code (excluding the import line itself)
        ...
        if (!classRegex.test(codeWithoutImports)) {
          unusedImportLines.add(index + 1); // Line numbers are 1-indexed
        }
      }
    }
  });
